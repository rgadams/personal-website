...
export class CanvasThreeDimensionsComponent implements OnInit {
    ...
    render(): void {
        ...
        displayObject.faces.sort((f1, f2) => (
            displayObject.vertices[f2[0] - 1].vector[2] +
        ...
        displayObject.faces.forEach((f) => {
            ...
            if (Vector.dotProduct(this.getFaceNormal(currentFace), this.camera.getOrientation()) < 0.01) {
                faces.push({
                    vertex1: this.translateObjectCoordinatesToScreenCoordinates(
                        displayObject.vertices[f[0] - 1],
                        this.cameraOn
                    ),
                    vertex2: this.translateObjectCoordinatesToScreenCoordinates(
                        displayObject.vertices[f[1] - 1],
                        this.cameraOn
                    ),
                    vertex3: this.translateObjectCoordinatesToScreenCoordinates(
                        displayObject.vertices[f[2] - 1],
                        this.cameraOn
                    ),
                    shading,
                } as Face);
    ...
    translateObjectCoordinatesToScreenCoordinates(vertex: Vector, camera = false): Vector {
        ...
        if (camera) {
            ...
            const transformedVertex = Vector.fromArray(
                perspectiveProjectionMatrix.multiply(vertexMatrix).transpose().matrix[0]
            );
    ...
    getFaceNormal(face: Face): Vector {
        const leftVector = Vector.fromTwoPoints(face.vertex1, face.vertex3);
        const rightVector = Vector.fromTwoPoints(face.vertex1, face.vertex2);
        return Vector.crossProduct(leftVector, rightVector);
    }
