...
export class SquareGameComponent implements OnInit {
    ...
    makeMove(index: number): void {
        ...
        const openIndex = this.grid.findIndex((value) => value === 16);
        // const moveDirection = this.isValidMove(index, openIndex);
        // if (moveDirection) {
        //     const gridElement = document.getElementById(index.toString(10));
        //     gridElement.setAttribute('moveDirection', moveDirection);
        //     delete this.error;
        //     setTimeout(() => {
        //         this.grid[openIndex] = this.grid[index];
        //         this.grid[index] = 16;
        //         gridElement.removeAttribute('moveDirection');
        //         if (this.isGameOver()) {
        ...
        if (this.isValidMoveV2(index, openIndex)) {
            ...
            setTimeout(() => {
                ...
                moves.indicesToMove.forEach((indice, i) => {
                    ...
                    if (i !== 0) {
                        // console.log(i, gridCopy[moves.indicesToMove[i - 1]]);
                        this.grid[indice] = gridCopy[moves.indicesToMove[i - 1]];
                    }
                });
                // console.log('Move to openIndex: ', moves.indicesToMove[moves.indicesToMove.length - 1]);
    ...
    isValidMove(index: number, openIndex: number): string {
        ...
        switch (openIndex % 4) {
            ...
            case 2: // third column
                validMove = (index === openIndex - 1 || index === openIndex + 1) || this.isValidVerticalMove(index, openIndex);
                break;
